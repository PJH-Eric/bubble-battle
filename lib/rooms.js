...
function createHub(opt) {
  ...
  const mapName = id => {
    ...
    return m ? m.name : '開闊草場';
  };

  const validMap = id => Maps.MAPS.some(m => m.id === id);

  function closeIfNoHumanPlayers(room) {
    if (!room || !rooms.has(room.id) || seats(room).length) return false;
  ...
  function createRoom(host, cfg) {
    ...
    const room = {
      ...
      mapId: validMap(cfg.mapId) ? cfg.mapId : 'open',
  ...
  function leave(roomId, personId) {
    ...
    if (!closeIfNoHumanPlayers(room)) balanceTeams(room);
    return room;
  }
  ...
  function updateRoom(roomId, hostId, cfg) {
    ...
    if (cfg.mapId === 'random') room.random = true;
    else if (cfg.mapId != null && validMap(cfg.mapId)) {
      room.mapId = cfg.mapId;
      room.random = false;
    }
  ...
  function start(roomId, hostId) {
    ...
    /* 倒數的時候場地就先蓋好、角色就先站上去，前端才有東西可以畫。
     * phase 留在 countdown，所以 tick 不會推進模擬、setInput 也收不進來，
     * 大家就只是站在起點上等數完。 */
    prepareMatch(room);
  ...
  function setInput(roomId, personId, input) {
    ...
    /* 記下「這包輸入是前端幾點送出、伺服器幾點收到」，快照會原封不動回傳，
     * 前端才算得出這張快照有多舊，本地預測該重播多少時間。 */
    if (typeof input.ct === 'number' && isFinite(input.ct)) {
      if (!room.lastStamp) room.lastStamp = new Map();
      room.lastStamp.set(personId, { ct: input.ct, at: now() });
    }
  }
  ...
  function snapshot(room, opts) {
    ...
    const snap = {
      ...
      bombs: m.bombs.map(b => ({
        id: b.id, owner: b.owner, c: b.c, r: b.r,
        px: +b.px.toFixed(3), py: +b.py.toFixed(3),
    ...
    if (room.phase === 'countdown') {
      snap.pre = true;                                          /* 還沒開打，前端不要做預測 */
      snap.countdownMs = Math.max(0, room.countdownUntil - now());
    }
    const stamp = opts.viewerId && room.lastStamp ? room.lastStamp.get(opts.viewerId) : null;
    if (stamp) {
      snap.ackCt = stamp.ct;                                   /* 前端自己的時戳（毫秒） */
      snap.ackAge = +((now() - stamp.at) / 1000).toFixed(4);   /* 收到後在伺服器待了多久（秒） */
    }
    if (m.phase !== 'playing') {
  ...
  return {
    ...
    rooms, invites,
    createRoom, listRooms, join, quickJoin, leave, markDisconnected, consumeClosedRooms,
